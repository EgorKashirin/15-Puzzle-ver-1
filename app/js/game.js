let buttonReset = document.getElementById('burron-reset');
let stepsCount = document.getElementById('steps');
var counter = 0;
...
function StartGame() {
    ...
    function event(x, y) {
        game.move(x, y);
        stepsCount.innerHTML = game.getClicks();
        context.fillRect(0, 0, canvas.width, canvas.height);
        game.draw();
        if (game.victory()) {
            alert("Вы Выиграли! Собрано за " + game.getClicks() + " касание!");
            game.mix(300);
            context.fillRect(0, 0, canvas.width, canvas.height);
